...
###
### geodesic evolution (derived from skeleton)
###
class SlicerVMTKEvolutionGeodesicGUI(SlicerVMTKAdvancedPageSkeleton):
    ...
    def UpdateMRML(self):
        ...
        if node:
            ...
            node.SetParameter('GEODESIC_iterations',self._iterationsThumbwheel.GetValue())
            node.SetParameter('GEODESIC_propagationScaling',self._propagationScalingSpinBox.GetWidget().GetValue())
            node.SetParameter('GEODESIC_curvatureScaling',self._curvatureScalingSpinBox.GetWidget().GetValue())
            node.SetParameter('GEODESIC_advectionScaling',self._advectionScaling.GetWidget().GetValue())
    ...
    def UpdateGUI(self):
        ...
        if node:
            ...
            if node.GetParameter('GEODESIC_iterations'):
                self._iterationsThumbwheel.SetValue(node.GetParameter('GEODESIC_iterations'))

            if node.GetParameter('GEODESIC_propagationScaling'):
                self._propagationScalingSpinBox.GetWidget().SetValue(node.GetParameter('GEODESIC_propagationScaling'))

            if node.GetParameter('GEODESIC_curvatureScaling'):
                self._curvatureScalingSpinBox.GetWidget().SetValue(node.GetParameter('GEODESIC_curvatureScaling'))

            if node.GetParameter('GEODESIC_advectionScaling'):
                self._advectionScalingSpinBox.GetWidget().SetValue(node.GetParameter('GEODESIC_advectionScaling'))
    ...
    def Execute(self):
        ...
        resultContainer = myLogic.ExecuteGeodesic(origInVolumeNode,inVolumeNode,numberOfIterations,propagationScaling,curvatureScaling,advectionScaling,self._parentClass._evolImageCheckbox.GetSelectedState())
