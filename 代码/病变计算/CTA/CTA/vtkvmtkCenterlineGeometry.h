...
class VTK_VMTK_COMPUTATIONAL_GEOMETRY_EXPORT vtkvmtkCenterlineGeometry : public vtkPolyDataAlgorithm
{
  ...
  vtkSetStringMacro(RadiusCurvatureArrayName);
  vtkGetStringMacro(RadiusCurvatureArrayName);

  vtkSetStringMacro(StenosisArrayName);
  vtkGetStringMacro(StenosisArrayName);
  ...
  static void ComputeRadiusCurvature(vtkPoints *points,vtkDataArray* radiusArray,vtkDoubleArray* radiusCurvatureArray, vtkDoubleArray* stenosisArray);
  ...
  char* RadiusCurvatureArrayName;
  char* StenosisArrayName;
